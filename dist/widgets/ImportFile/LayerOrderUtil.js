...
define(['dojo/_base/declare', 'dojo/_base/array', 'dojo/Deferred', 'jimu/LayerInfos/LayerInfos'], function (declare, array, Deferred, LayerInfos) {
  return declare(null, {
    config: null,

    constructor: function constructor(config, map) {
      this.config = config;
      this.map = map;
      if (!('layerOrder' in this.config) || this.config.layerOrder.length === 0) {
        //get all param whose type is GPFeatureRecordSetLayer
        this.config.layerOrder = this.getCandidateParamNames(false);
      }
    },
     ...
     */
    getCandidateParamNames: function getCandidateParamNames(orderable) {
      var candidateLayers = [];
      ...
      array.forEach(this.config.outputParams, function (param) {
        if (param.dataType === 'GPFeatureRecordSetLayer' && (!orderable || this.config.useResultMapServer !== true)) {
      ...
      array.forEach(this.config.inputParams, function (param) {
        if (param.dataType === 'GPFeatureRecordSetLayer' && (!orderable || param.featureSetMode === 'draw')) {
          candidateLayers.push(param.name);
        }
      }, this);
     ...
     */
    getOrderableInput: function getOrderableInput() {
      var candidateLayers = [];
      ...
      //input is under the output
      array.forEach(this.config.inputParams, function (param) {
        if (param.dataType === 'GPFeatureRecordSetLayer' && param.featureSetMode === 'draw') {
          candidateLayers.push(param.name);
        }
      }, this);
     ...
     */
    getOrderableOutput: function getOrderableOutput() {
      var candidateLayers = [];
      ...
      //input is under the output
      array.forEach(this.config.outputParams, function (param) {
        if (param.dataType === 'GPFeatureRecordSetLayer' && this.config.useResultMapServer !== true) {
          candidateLayers.push(param.name);
        }
      }, this);
     ...
     */
    calculateLayerIndex: function calculateLayerIndex(paramName, widgetUID) {
      if (!this.map) {
      ...
      if (idx === -1) {
        ...
        for (i = idx - 1; i >= 0; i--) {
          targetIndex = array.indexOf(this.map.graphicsLayerIds, widgetUID + params[i]);
          if (targetIndex !== -1) {
        ...
        if (!def.isResolved()) {
          LayerInfos.getInstance(this.map, this.map.itemInfo).then(function (layerInfosObject) {
            ...
            if (mapNotesLayerArray.length === 0) {
              ...
              targetIndex = array.indexOf(this.map.graphicsLayerIds, targetLayerId);
              def.resolve(targetIndex);
            }
          });
        }
