...
define(['dojo/_base/declare', 'dojo/_base/lang', 'dojo/_base/array', 'dojo/dom-style', 'dojo/dom-attr', 'dojo/on', 'dojo/Deferred', 'dojo/text!./RasterLayerEditor.html', 'dijit/_TemplatedMixin', 'dijit/_WidgetsInTemplateMixin', 'esri/tasks/RasterData', 'esri/request', 'jimu/dijit/ViewStack', 'jimu/dijit/Message', '../BaseEditor', 'dijit/form/TextBox', 'dijit/form/ValidationTextBox'], function (declare, lang, array, domStyle, domAttr, on, Deferred, template, _TemplatedMixin, _WidgetsInTemplateMixin, RasterData, esriRequest, ViewStack, Message, BaseEditor) {
  ...
  return declare([BaseEditor, _TemplatedMixin, _WidgetsInTemplateMixin], {
    ...
    parseDefaultValue: function parseDefaultValue() {
      var tokens;

      if (this.value.indexOf(ITEM_ID_DELIMITER) === 0) {
        this.mode = MODE_ITEM;
      } else {
        if (this.value.indexOf(URL_DELIMITER) === 0 || this.value.indexOf(FORMAT_DELIMITER) === 0) {
    ...
    getValue: function getValue() {
      if (this.mode === MODE_URL) {
        return URL_DELIMITER + this._getUrl() + ',' + FORMAT_DELIMITER + this.rasterFormatInput.get('value');
    ...
    _onUrlModeSelect: function _onUrlModeSelect() {
      this.mode = MODE_URL;
      if (this.viewStack) {
    ...
    _onItemModeSelect: function _onItemModeSelect() {
      this.mode = MODE_ITEM;
      if (this.viewStack) {
    ...
    _onFileInputClick: function _onFileInputClick() {
