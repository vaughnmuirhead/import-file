define(['dojo/_base/declare', 'dojo/_base/lang', 'dojo/_base/array', 'dojo/_base/html', 'dojo/dom-style', 'dojo/on', 'dijit/_TemplatedMixin', 'esri/layers/GraphicsLayer', 'esri/layers/FeatureLayer', 'esri/graphicsUtils', 'esri/renderers/SimpleRenderer', 'esri/renderers/jsonUtils', 'esri/InfoTemplate', 'jimu/exportUtils', 'jimu/dijit/ExportChooser', 'dojo/text!./FeatureSetRenderer.html', '../BaseResultRenderer', '../LayerOrderUtil', './defaultSymbol'], function (declare, lang, array, html, domStyle, on, _TemplatedMixin, GraphicsLayer, FeatureLayer, graphicsUtils, SimpleRenderer, rendererUtils, InfoTemplate, exportUtils, ExportChooser, template, BaseResultRenderer, LayerOrderUtil, defaultSymbol) {
  return declare([BaseResultRenderer, _TemplatedMixin], {
    ...
    _displayText: function _displayText() {
    ...
    _drawResultFeature: function _drawResultFeature(param, featureset) {
      if (this.config.shareResults) {
        if (!param.defaultValue || !param.defaultValue.geometryType) {
          throw Error('Output parameter default value does not provide enough information' + ' to draw feature layer.');
        }
      ...
      if (param.popup.enablePopup) {
        ...
        infoTemplate = new InfoTemplate(param.popup.title || '${Non-Exist-Field}', this._generatePopupContent(featureset));
      }
      for (var i = 0; i < len; i++) {
        if (infoTemplate) {
    ...
    _addResultLayer: function _addResultLayer(paramName) {
      var layerOrderUtil = new LayerOrderUtil(this.config, this.map);
      try {
        layerOrderUtil.calculateLayerIndex(paramName, this.widgetUID).then(lang.hitch(this, function (layerIndex) {
          if (layerIndex !== -1) {
    ...
    _generatePopupContent: function _generatePopupContent(featureset) {
      var str = '<div class="geoprocessing-popup">' + '<table class="geoprocessing-popup-table" ' + 'cellpadding="0" cellspacing="0">' + '<tbody>';
      var rowStr = '';
      var fields;
      ...
      array.forEach(fields, function (field) {
        var row = '<tr valign="top">' + '<td class="attr-name">' + field.alias + '</td>' + '<td class="attr-value">${' + field.name + '}</td>' + '</tr>';
        rowStr += row;
      });
