...
define(['dojo/_base/array', 'dojo/Deferred', 'esri/request'], function (array, Deferred, esriRequest) {
  var mo = {};
  ...
  mo.allowShareResult = function (config) {
    return array.some(config.outputParams, function (param) {
      return param.dataType === 'GPRecordSet' || param.dataType === 'GPFeatureRecordSetLayer' && param.defaultValue && param.defaultValue.geometryType;
    });
  };
  ...
  function getGPServerDescription(gpTaskUrl) {
    ...
    return esriRequest(args).then(function (serverDescription) {
      ...
      serverInfo.hasResultMapServer = serverDescription.executionType === "esriExecutionTypeAsynchronous" && 'resultMapServerName' in serverDescription && serverDescription.resultMapServerName !== '';
      return serverInfo;
    });
  }
